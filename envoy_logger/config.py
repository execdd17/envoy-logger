...
class Config:
    def __init__(self, data: Dict[str, Any], database: str) -> None:
        try:
            self.enphase_email: str = os.environ.get("ENPHASE_EMAIL", data["enphaseenergy"]["email"])
            self.enphase_password: str = os.environ.get("ENPHASE_PASSWORD", data["enphaseenergy"]["password"])
            ...
            match database:
                case "influxdb":
                    ...
                    self.influxdb_token: str = os.environ.get("INFLUXDB_TOKEN", data["influxdb"]["token"])
